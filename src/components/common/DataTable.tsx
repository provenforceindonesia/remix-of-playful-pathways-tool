...
export type Column<T> = {
  ...
  /**
   * Tetap dipertahankan agar tidak menyebabkan error
   * pada file lain yang mungkin masih mengirim property align.
   * Alignment tabel saat ini dibuat global rata kiri.
   */
...
export function DataTable<T extends Record<string, unknown>>({
  ...
  const exportCsv = () => {
    ...
    const body = filtered
      .map((row) => columns.map((column) => `"${String(val(row, column) ?? "").replace(/"/g, '""')}"`).join(","))
      .join("\n");
  ...
  return (
    <Card className="gap-0 overflow-hidden p-0">
      {/* Toolbar */}
      {(searchable || exportName || toolbar || toolbarActions) && (
      ...
      {filtered.length === 0 ? (
        ...
        <>
            ...
            <table className="w-full min-w-max text-sm">
                ...
                <tr className="border-b border-border/70 bg-muted/40">
                  ...
                  {columns.map((column) => (
                    <th
                      key={column.key}
                      className={cn(
                        "px-3 py-2.5 text-left text-xs font-semibold tracking-wide text-muted-foreground uppercase whitespace-nowrap",
                        column.className,
                      )}
                    >
                      {column.sortable === false ? (
                        ...
                        <button
                          type="button"
                          className="inline-flex items-center justify-start gap-1 text-left hover:text-primary"
                          onClick={() =>
                            setSort((currentSort) =>
              ...
              <tbody>
                {view.map((row, index) => (
                  ...
                  >
                    {/* Nomor */}
                    <td className="tabular px-3 py-2.5 text-left text-muted-foreground whitespace-nowrap">
                    ...
                    {columns.map((column) => (
                      <td
                        key={column.key}
                        className={cn("tabular px-3 py-2.5 text-left whitespace-nowrap", column.className)}
                      >
                        {column.render ? column.render(row) : String(val(row, column) ?? "-")}
                      </td>
                    ))}
                  </tr>
                ))}
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-t border-border/70 px-3 py-2 text-xs text-muted-foreground">
            <span>
              Menampilkan {current * pageSize + 1}–{Math.min((current + 1) * pageSize, filtered.length)} dari{" "}
