...
type StoredPlanItem = {
  ...
  routings?: { code?: string; name?: string } | null;
};
...
function PlansPage() {
  ...
  const compatibleRoutings = (productId: string, variantId: string | null) =>
    routingRows.filter(
      (routing) =>
        String(routing.status ?? "Active") === "Active" &&
        String(routing.product_id ?? "") === productId &&
        (!routing.variant_id || String(routing.variant_id) === String(variantId ?? "")),
    );

  const defaultRouting = (productId: string, variantId: string | null) => {
    const compatible = compatibleRoutings(productId, variantId);
    return compatible.find((routing) => String(routing.variant_id ?? "") === String(variantId ?? "")) ?? compatible[0];
  };
  ...
  const calculateManpower = (item: PlanItemInput) => {
    ...
    const requiredMinutes = target * laborMinutesPerUnit;
    const routingMinimum = operations.reduce(
      (maximum, operation) => Math.max(maximum, numberValue(operation.manpower)),
      0,
    );
    const canCalculate = target > 0 && available > 0 && laborMinutesPerUnit > 0;
    const calculated = canCalculate ? Math.ceil(requiredMinutes / available) : 0;

    return {
      available,
      recommended: canCalculate ? Math.max(calculated, routingMinimum) : 0,
    };
  };
  ...
  const buildPlanItems = (salesOrderId: string, currentPlanId?: string): PlanItemInput[] => {
    ...
    return (salesOrder.sales_order_items ?? []).map((item) => {
      ...
      const automaticRouting = defaultRouting(
      ...
      return {
        ...
        machine_id: "",
        shift_id: "",
        routing_id: automaticRouting ? String(automaticRouting.id) : "",
        available_minutes: 0,
      ...
      render: (r) =>
        stacked(r, (item) => (
          <div className="min-w-32">
            <p>{item.routings?.code ?? "—"}</p>
            <p className="text-xs text-muted-foreground">{item.routings?.name ?? "Routing standar"}</p>
          </div>
        )),
    },
    {
    ...
    },
    {
      ...
      render: ({ value, setValue }) => {
        ...
        const updateItem = (index: number, patch: Partial<PlanItemInput>) => {
          ...
          const automaticRouting = defaultRouting(previous.product_id, previous.variant_id);
          let updated = {
            ...previous,
            ...patch,
            routing_id: previous.routing_id || (automaticRouting ? String(automaticRouting.id) : ""),
          };
          const result = calculateManpower(updated);
          const isManualManpowerChange = Object.prototype.hasOwnProperty.call(patch, "planned_manpower");
          ...
          updated = {
            ...
            planned_manpower:
              !isManualManpowerChange && followedRecommendation ? result.recommended : updated.planned_manpower,
          };
          next[index] = updated;
          setValue(next);
        };
        ...
        return (
          <div className="space-y-4">
            {items.map((item, index) => {
              const itemRoutings = compatibleRoutings(item.product_id, item.variant_id);
              const selectedRouting = itemRoutings.find((routing) => String(routing.id) === item.routing_id);
              const selectedRoutingOperations = ((selectedRouting?.routing_operations ?? []) as Row[])
                .slice()
                .sort((a, b) => numberValue(a.seq) - numberValue(b.seq));
              const routingProcess = selectedRoutingOperations
                .map((operation) => String(operation.operation_name ?? "").trim())
                .filter(Boolean)
                .join(" → ");
              const hasRoutingStandard = selectedRoutingOperations.some(
                (operation) => numberValue(operation.standard_cycle_time_sec) > 0,
              );
              const routingMachineIds = new Set(
                ((selectedRouting?.routing_operations ?? []) as Row[])
                  .map((operation) => String(operation.machine_id ?? ""))
                  .filter(Boolean),
              );
              const filteredMachines = machineRows.filter((machine) => {
                const matchesLine = !item.line_id || String(machine.line_id ?? "") === item.line_id;
                const matchesRouting = routingMachineIds.size === 0 || routingMachineIds.has(String(machine.id));
                return matchesLine && matchesRouting;
              });
                ...
                <div key={item.sales_order_item_id} className="rounded-xl border bg-card p-4">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-1.5">
                      ...
                      <div className="flex items-center gap-2">
                        <Input
                          ...
                          onChange={(event) => {
                            updateItem(index, {
                              target_qty: Math.min(numberValue(event.target.value), item.max_target_qty),
                            });
                          }}
                        />
                    ...
                    <div className="space-y-1.5">
                      ...
                      {itemRoutings.length > 1 ? (
                        <Select
                          value={item.routing_id}
                          disabled={fullyPlanned}
                          onValueChange={(value) => updateItem(index, { routing_id: value, machine_id: "" })}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder="Pilih routing" />
                          </SelectTrigger>
                          <SelectContent>
                            {itemRoutings.map((routing, routingIndex) => (
                              <SelectItem key={String(routing.id)} value={String(routing.id)}>
                                {String(routing.code ?? "Routing")} — {String(routing.name ?? "Routing produk")}
                                {routingIndex === 0 ? " — Utama" : " — Alternatif"}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                        ...
                        <div className="rounded-md border bg-muted/30 px-3 py-2 text-sm">
                          {selectedRouting
                            ? `${String(selectedRouting.code ?? "Routing")} — ${String(selectedRouting.name ?? "Routing produk")}`
                            : "Belum ada routing aktif"}
                        </div>
                      )}
                      <p className="text-xs text-muted-foreground">
                        {itemRoutings.length > 1
                          ? "Pilih routing alternatif hanya jika jalur utama tidak digunakan."
                          : routingProcess || "Routing standar produk belum memiliki urutan proses."}
                      </p>
                    </div>
                    <div className="space-y-1.5 sm:col-span-2">
                      ...
                      />
                      <p className="text-xs text-muted-foreground">
                        {numberValue(item.target_qty) <= 0
                          ? "Isi Target Plan dan Shift untuk menghitung rekomendasi manpower."
                          : !item.shift_id
                            ? "Pilih Shift untuk menghitung waktu tersedia dan rekomendasi manpower."
                            : !selectedRouting || !hasRoutingStandard
                              ? "Rekomendasi belum tersedia karena standard cycle time routing belum lengkap."
                              : item.recommended_manpower > 0
                                ? `Rekomendasi: ${formatNumber(item.recommended_manpower)} orang berdasarkan target ${formatNumber(item.target_qty)} ${item.uom_code}, routing ${String(selectedRouting.code ?? "-")}, dan waktu tersedia ${formatNumber(item.available_minutes)} menit. Jumlah ini tetap dapat diubah.`
                                : "Rekomendasi manpower belum dapat dihitung."}
                      </p>
                    </div>
                    <div className="space-y-1.5">
    ...
    <CrudPage<Row>
      ...
      beforePayload={(values) => {
        ...
        for (const item of selectedItems) {
          ...
          if (!item.planned_date || !item.line_id || !item.machine_id || !item.shift_id) {
            throw new Error(`Lengkapi tanggal, line, mesin, dan shift untuk ${item.product_name}.`);
          }
